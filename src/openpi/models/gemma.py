...
class RMSNorm(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        if modulation.ndim == x.ndim - 1:
            # One conditioning vector for the complete token sequence.
            modulation = modulation[:, None, :]
        elif modulation.ndim != x.ndim:
            raise ValueError(
                f"adaRMS conditioning rank must match the input rank or be one smaller, got {cond.shape=} and {x.shape=}"
            )
